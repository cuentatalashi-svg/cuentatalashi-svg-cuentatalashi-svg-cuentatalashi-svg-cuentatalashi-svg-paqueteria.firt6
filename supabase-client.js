...
function mapToSupabaseFields(obj, tableName) {
  if (tableName === 'paquetes') {
    ...
    delete mapped.fotoRecibidoPor;
    delete mapped.entregadoPor;
    delete mapped.fotoEntregadoPor;
      ...
      for (const remoteItem of mappedRemoteData) {
        try {
          ...
          if (!existingLocal) {
            ...
            // CRÍTICO: Eliminar 'id' de Supabase porque IndexedDB usa autoincrement
            const { id, ...itemWithoutId } = remoteItem;
            await addItem(tableName, itemWithoutId);
            console.log(`[Sync] Item remoto agregado a ${tableName}`);
          } else {
            // Existe - SOLO actualizar si datos remotos son más nuevos
            // Comparar timestamp (entregadoEn/entregadoen para paquetes, created para otros)
            let shouldUpdate = false;

            if (tableName === 'paquetes') {
              // Para paquetes, comparar por entregadoen (si existe)
              const localTimestamp = existingLocal.entregadoEn || existingLocal.created || 0;
              const remoteTimestamp = remoteItem.entregadoEn || remoteItem.created || 0;
              shouldUpdate = remoteTimestamp > localTimestamp;
            } else {
              // Para otras tablas, asumir que remoto es fuente de verdad
              shouldUpdate = true;
            }

            if (shouldUpdate) {
              await putItem(tableName, { ...existingLocal, ...remoteItem, id: existingLocal.id });
              console.log(`[Sync] Item local actualizado en ${tableName} (remoto más nuevo)`);
            } else {
              console.log(`[Sync] Item local más reciente en ${tableName}, no actualizar`);
            }
          }
        } catch (err) {
...
/**
 * Admin helper: Genera un código de invitación para una organización
 */
async function generateInviteCode(orgId) {
  if (!supabaseClient || !orgId) return null;
  try {
    const { data, error } = await supabaseClient.rpc('generate_invite_code', { org_id: orgId });
    if (error) {
      console.error('[generateInviteCode] Error:', error);
      return null;
    }
    return data; // Retorna el código generado
  } catch (err) {
    console.error('[generateInviteCode] Exception:', err);
    return null;
  }
}

/**
 * Admin helper: Obtiene todos los códigos de invitación de una organización
 */
async function getInviteCodes(orgId) {
  if (!supabaseClient || !orgId) return [];
  try {
    ...
    console.error('[getInviteCodes] Exception:', err);
    return [];
  }
}

/**
 * Admin helper: Obtiene todos los guardias de una organización
 */
async function getOrgGuards(orgId) {
  if (!supabaseClient || !orgId) return [];
  try {
    const { data, error } = await supabaseClient
      .from('users')
    ...
    if (error) {
      console.error('[getOrgGuards] Error:', error);
      return [];
    }
    return data || [];
  } catch (err) {
    console.error('[getOrgGuards] Exception:', err);
    return [];
  }
}

/**
 * Elimina un paquete de Supabase por su guía y organization_id.
 * @param {string} guia - Número de guía del paquete
 * @param {string} orgId - ID de la organización
 * @returns {Promise<boolean>} - true si se eliminó correctamente, false en caso contrario
 */
async function deletePackageFromSupabase(guia, orgId) {
  if (!supabaseClient || !guia || !orgId) return false;
  if (!navigator.onLine) {
    console.warn('[Delete] Offline, no se puede eliminar de Supabase');
    return false;
  }
  ...
  try {
    ...
    if (error) {
      console.error('[Delete] Error eliminando paquete:', error.message);
      return false;
    }

    console.log(`[Delete] ✅ Paquete ${guia} eliminado de Supabase`);
    return true;
  } catch (err) {
