  ...
  if (document.body.classList.contains('page-main')) {
    ...
    async function showScreen(id) {
      ...
      if (id === 'screen-paqueteria') { mainContainer.classList.add('show-paqueteria'); }
        ...
        // Refresh admin data
        if (typeof window.refreshAdminData === 'function') await window.refreshAdminData();
      }
    ...
    // --- ADMIN PANEL ---
    if (userRol === 'admin') {
      const orgIdText = document.getElementById('orgIdText');
      ...
      // Mostrar org ID
      if (orgIdText) orgIdText.textContent = user.organization_id;

      // Copiar org ID
      if (copyOrgIdBtn) {
        copyOrgIdBtn.onclick = async () => {
          try {
            await navigator.clipboard.writeText(user.organization_id);
            showMessage('Organization ID copiado', 'success');
          } catch (err) {
      ...
      if (generateCodeBtn) {
        generateCodeBtn.onclick = async () => {
          try {
            showMessage('Generando código...', 'loading', 0);
            const code = await generateInviteCode(user.organization_id);
            if (code) {
              hideToast();
              alert(`✅ Código generado exitosamente:\n\n${code}\n\nVálido por 7 días, un solo uso.`);
              await refreshCodes();
            } else {
              showMessage('Error al generar código', 'error');
            }
          } catch (err) {
            console.error('[Admin] Error generando código:', err);
      ...
      async function refreshCodes() {
        ...
        try {
          const codes = await getInviteCodes(user.organization_id);
          if (!codes || codes.length === 0) {
            codesTableBody.innerHTML = '<tr><td colspan="4" style="padding: 16px; text-align: center;" class="muted">No hay códigos generados</td></tr>';
            return;
          }
          codesTableBody.innerHTML = codes.map(c => {
            const creado = new Date(c.created_at).toLocaleDateString();
            const expira = new Date(c.expires_at).toLocaleDateString();
            const estado = c.used ? '✅ Usado' : '⏳ Disponible';
            const estadoClass = c.used ? 'muted' : 'success';
            return `<tr>
              <td style="padding: 8px; font-family: monospace; font-weight: bold;">${c.code}</td>
              <td style="padding: 8px;">${creado}</td>
              <td style="padding: 8px;">${expira}</td>
              <td style="padding: 8px; text-align: center;" class="${estadoClass}">${estado}</td>
            </tr>`;
          }).join('');
        } catch (err) {
          console.error('[Admin] Error refrescando códigos:', err);
          codesTableBody.innerHTML = '<tr><td colspan="4" style="padding: 16px; text-align: center;" class="muted">Error cargando códigos</td></tr>';
        }
      }

      // Refrescar lista de guardias
      async function refreshGuards() {
        if (!guardsTableBody) return;
        try {
          const guards = await getOrgGuards(user.organization_id);
          if (!guards || guards.length === 0) {
            guardsTableBody.innerHTML = '<tr><td colspan="3" style="padding: 16px; text-align: center;" class="muted">No hay guardias registrados</td></tr>';
            return;
          }
          guardsTableBody.innerHTML = guards.map(g => {
            const registro = g.created_at ? new Date(g.created_at).toLocaleDateString() : '-';
            return `<tr>
              <td style="padding: 8px;">${g.nombre || '-'}</td>
              <td style="padding: 8px;">${g.email || '-'}</td>
              <td style="padding: 8px;">${registro}</td>
            </tr>`;
          }).join('');
        } catch (err) {
          console.error('[Admin] Error refrescando guardias:', err);
          guardsTableBody.innerHTML = '<tr><td colspan="3" style="padding: 16px; text-align: center;" class="muted">Error cargando guardias</td></tr>';
        }
      }

      // Exponer función de refresh para que showScreen la pueda llamar
      window.refreshAdminData = async function () {
        await Promise.all([refreshCodes(), refreshGuards()]);
      };

      // Refresh inicial si ya estás en admin
    ...
    deleteConfirmBtn.addEventListener('click', async () => {
      ...
      try {
        if (itemToDelete.type === 'paquete') {
          const paquete = await getByKey('paquetes', itemToDelete.id);
          if (paquete) {
            // Eliminar de Supabase primero
            if (typeof deletePackageFromSupabase === 'function' && user.organization_id) {
              await deletePackageFromSupabase(paquete.guia, user.organization_id);
            }
    ...
    // --- Funciones Admin Legacy (DESHABILITADO - reemplazado por nuevo panel) ---
    /*
